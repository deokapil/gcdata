...
strain = read.table("UCI HAR Dataset/train/subject_train.txt", col.names=c("subject_id"))

# assign row number as the values of ID column
strain$ID <- as.numeric(rownames(strain))
...
# merge strain and y_train to train
train <- merge(strain, y_train, all=TRUE)
